...
def test_compute_channel_selection_outside_dataset_range(mock_tile_client, mock_large_image, mock_gaussian_filter):
    ...
    parses cleanly and then matches no frame: the worker would blur nothing and
...
def test_compute_channel_selection_partially_outside_range_warns(mock_tile_client, mock_large_image, mock_gaussian_filter):
    ...
    # Channel 0 still gets blurred: 2 of the fixture's 4 frames.
    assert mock_gaussian_filter.call_count == 2
    mock_large_image.write.assert_called_once_with('/tmp/output.tiff')
