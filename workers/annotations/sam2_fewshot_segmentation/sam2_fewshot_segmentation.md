...
Uses SAM2 features for few-shot segmentation based on training annotations
...
| **Points per side** | `number` |  | 128 | 16 - 256 |  |
...
```bash
./build_workers.sh sam2_fewshot_segmentation
```

## Testing

```bash
./build_workers.sh --build-and-run-tests sam2_fewshot_segmentation
```
