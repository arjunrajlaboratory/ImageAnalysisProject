#!/usr/bin/env python3
"""
Local experiment script for the SAM2 few-shot segmentation worker.

Splits annotated data into train/test, runs the SAM2 pipeline on CPU,
...
Usage:
    ...
    python test_sam2_fewshot_experiment.py --dataset DATASET_ID --tag "DAPI blob"
...
def encode_image_with_sam2(predictor, image_np):
    """Encode an image crop using SAM2's image encoder."""
...
def run_experiment(args):
    ...
    print("\n" + "=" * 60)
    print("SAM2 FEW-SHOT SEGMENTATION EXPERIMENT")
    print("=" * 60)
    ...
    # ── Group by location ──
    # Group annotations by (XY, Z, Time) so we can do per-frame evaluation
    ...
    # Group test annotations by location for per-frame evaluation
    ...
    # Only run inference on locations that have test annotations
    ...
    all_gt_polys = []
    all_pred_polys = []
    all_images = {}  # Cache for visualization
    ...
    for loc_idx, (xy, z, t) in enumerate(test_locations):
        ...
        t0 = time.time()
        # SAM2 expects float32 input in [0, 255] range
    ...
    if args.visualize and len(all_images) > 0:
        ...
        # Use the first test location's image for visualization
        first_loc = test_locations[0]
        img = all_images[first_loc]

        # Get GT and pred polys for this location only
        ...
        # Rebuild predictions for this one location (already computed above
        # but not stored separately; reuse the overall matches for simplicity)
        # For visualization, show overall results on first frame
...
def main():
    parser = argparse.ArgumentParser(
        description='SAM2 few-shot segmentation experiment',
    ...
    # SAM2 parameters
    parser.add_argument('--model', type=str, default='sam2.1_hiera_small.pt',
                        help='SAM2 checkpoint name')
    parser.add_argument('--checkpoint-dir', type=str,
                        default='/tmp/sam2/checkpoints',
