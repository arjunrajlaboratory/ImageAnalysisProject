...
description: Diagnose and fix the recurring robustness failure modes of NimbusImage workers in this repository, then sweep every sibling worker for the same latent bug. Use when a worker crashes or errors in a deploy/production run, when the user pastes a worker stack trace (KeyError 'IndexRange', AttributeError on .exterior/.items(), bare KeyError from coordinatesToFrameIndex, ModuleNotFoundError: pkg_resources), when asked to "harden", "make robust", "fix and apply to all workers", "audit the other workers for this", or before shipping a worker that handles multi-dimensional data, channelCheckboxes, polygon geometry, batch coordinates, or heavy ML dependencies. The defining behavior: it does not just patch the one worker that failed — it generalizes the failure into a known pattern and audits/fixes every other worker susceptible to it, adding a test to the shared helper so the class of bug does not recur. Scope boundary: this is for fixing/hardening EXISTING workers. To create a brand-new worker from scratch use nimbus-worker-scaffold; for general runtime API reference (image access, annotation CRUD, coordinate conventions) use nimbus-interface.
---
